...
export default function ExperienceSection({ initialData }: { initialData?: ExperienceItem[] }) {
  ...
  return (
    <section id="experience" className="relative py-20 pb-28 sm:pb-20 scroll-mt-20 bg-[#070612] text-white border-b border-zinc-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          {experiences.map((exp, index) => (
            <div
              key={exp._id || index}
              className="group relative rounded-3xl p-[1.5px] overflow-hidden transition-all duration-300 hover:scale-[1.01] bg-zinc-900/40 shadow-xl"
            >
              ...
              {/* Static Border */}
              <div className="absolute inset-0 rounded-3xl border border-zinc-800/80 group-hover:border-transparent transition-colors duration-200 pointer-events-none" />

              {/* Inner Card */}
              <div className="relative z-10 rounded-[calc(1.5rem-1.5px)] bg-[#0c0d16] p-6 sm:p-8 space-y-5">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 pt-1 text-sm font-semibold">
                      <span className="text-[#5AB2FF] font-mono">{exp.company}</span>
                      {exp.location && (
                  ...
                  {/* Time Bound Badge */}
                  <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-300 text-xs font-mono">
                ...
                {exp.technologies && exp.technologies.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2 pt-2">
                    <span className="text-xs font-mono text-zinc-400 mr-1 flex items-center gap-1">
                      <Code2 className="w-3.5 h-3.5 text-[#5AB2FF]" />
                      Tech:
                    </span>
                    {exp.technologies.map((tech, i) => (
                      <span
                        key={i}
                        className="px-2.5 py-1 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-300 hover:border-purple-500/50 hover:text-purple-200 text-[11px] font-mono transition-colors"
                      >
                        {tech}
                      </span>
                    ))}
